...
const BlogDetails = () => {
  ...
  useEffect(() => {
    const fetchBlogData = async () => {
      try {
        ...
        // Handle data wrapping structure identical to the home feed payload
  ...
  // Dynamic property discovery to check for all possible backend image keys
  const targetImage = blog.image || blog.imageUrl || blog.coverImage;
        ...
        {targetImage && (
          <div className="w-full aspect-[16/10] mb-12 bg-neutral-100 dark:bg-neutral-900 overflow-hidden">
            <img 
              ...
              onError={(e) => {
                // Fail-safe handler: if the production image string fails to load
                console.warn("Header asset string broken or inaccessible via network paths.");
              }}
            />
          </div>
        )}
